...
make catalog                          # (re)build the live git-backed store → ./catalog
...
The `catalog/` directory committed in this repo is a **static snapshot** so you can browse the
entries (`catalog.yaml` + `metadata/*.yaml`) without running anything. It has no inner `.git`;
`make catalog` rebuilds the live, git-backed store in place (then `cd catalog && git log` shows
the per-entry version history). Because that live rebuild gives `catalog/` its own `.git`, the
committed snapshot is effectively frozen — to refresh it, rebuild then strip the inner `.git` so
it commits as plain files again: `make catalog && rm -rf catalog/.git && git add -f catalog`.

Curated aliases: `dbt-star-velocity`, `dbt-download-trend`, `dbt-momentum`, `duckdb-buzz`,
`dbt-mentions`, `dbt-health`, `rising-tools` (see `CATALOG_ENTRIES` in `expr.py`). Each binds
one metric to a demo tool — `dbt-core`, `duckdb` — that the digest *measures*; these are
analysed tools, **not** project dependencies.
...
scripts/catalog_register.py   # build + register curated entries (make catalog)
catalog/             # the xorq catalog store — git-backed, regenerated by make catalog
